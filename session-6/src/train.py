...
def train():
    ...
    train_X, train_y = torch.from_numpy(train_X.copy()).float(), torch.from_numpy(train_y.copy()).float().unsqueeze(1)
    test_X, test_y = torch.from_numpy(test_X.copy()).float(), torch.from_numpy(test_y.copy()).float().unsqueeze(1)
